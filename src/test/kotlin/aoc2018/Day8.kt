...
class Day8(test: Int? = null) : Day(test) {
    ...
    private fun Node.sum(): Int = metadata.sum() + children.sumOf { it.sum() }

    override fun solvePart2() = root.value()

    private fun Node.value(): Int =
        if (children.isEmpty()) metadata.sum() else metadata.sumOf { children.getOrNull(it - 1)?.value() ?: 0 }

    private val root = input.split(" ").map { it.toInt() }.parse(0).first
    ...
    private data class Node(val metadata: List<Int>, val children: List<Node>)
}
