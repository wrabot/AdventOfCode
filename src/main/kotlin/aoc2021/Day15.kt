...
class Day15(test: Int? = null) : Day(2021, 15, test) {
    ...
    private fun minRisk(cave: Board<Cell>) = shortPath(cave.points.first(), cave.points.last(), { _, destination -> cave[destination].risk }) { cave.neighbors4(it) }.drop(1).sumOf { cave[it].risk }
}
